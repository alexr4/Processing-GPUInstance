...
void main(){
    ...
    float dist = snoise(offset.xyz * 0.0025 + time * 0.1);
    dist = clamp(dist, 0.0, 1.0);
    float isHidden = smoothstep(0.0, 0.1, dist);
    vec3 growthVertex = vertex.xyz * vec3(1.0, dist * maxSize, 1.0);
    ...
    vec4 position = vec4(rotatedVertex.xyz + offset.xyz - direction.xyz * 5.0, 1.0);
