...
public class VBOInterleaved implements ShaderSource {
  ...
  private GL4 gl;
  private int maxInstance = round(pow(10, 3));
  ...
  private void initVBO(PGraphics context, int numberOfVertex) {
    ...
    this.VBOi               = new float[vertexCount * (VRT_CMP_COUNT + CLR_CMP_COUNT)];
    this.VBOOffset          = new float[maxInstance * VRT_CMP_COUNT];
  ...
  private void feedOffsetBuffer() {
    ...
    for (int i=0; i<VBOOffset.length / VRT_CMP_COUNT; i++) {
      VBOOffset[i * VRT_CMP_COUNT + 0] = float( i / (divider * divider)) * offset - centerOffset;
      VBOOffset[i * VRT_CMP_COUNT + 1] = float((i/divider) % divider) * offset - centerOffset;
      VBOOffset[i * VRT_CMP_COUNT + 2] = float( i % divider ) * offset - centerOffset;
      VBOOffset[i * VRT_CMP_COUNT + 3] = 0.0;
    }
  }
  ...
  public void draw(PGraphics context) {
    ...
    gl.glVertexAttribPointer(vrtLoc, VRT_CMP_COUNT, PGL.FLOAT, false, STRIDE, VRT_OFFSET);
    gl.glVertexAttribPointer(clrLoc, CLR_CMP_COUNT, PGL.FLOAT, false, STRIDE, CLR_OFFSET);
    ...
    //Associate current bound vbo with attribute
    gl.glVertexAttribPointer(offsetLoc, VRT_CMP_COUNT, PGL.FLOAT, false, VRT_CMP_COUNT * Float.BYTES, VRT_OFFSET);
    gl.glVertexAttribDivisor(offsetLoc, 1);

    //draw buffer
    gl.glDrawArraysInstanced(PGL.TRIANGLES, 0, this.vertexCount, maxInstance);
    //gl.glDrawArrays(PGL.TRIANGLES, 0, this.vertexCount);
